...
//        System.out.println(li_list.size());
        ...
        // ul 태그의 자식 태그인 li 태그의 자식 태그가 class 값이 item_issue인
        // div 태그 하나 밖에 없으므로 ul 태그에서 div 태그를 바로 검색해서 모두 가져옴 (총 20개)
        ...
        for (int i = 0; i < item_issue.size(); i++) {
            ...
            // class 가 item_issue 인 태그 안에서  class 가 cont_thumb 인 태그는 1개 밖에 없음
...
//            // strong 태그가 1개 밖에 없음, first()를 사용하여 1개만 가져옴
//            Element strong = cont_thumb.select("strong").first();
//            // strong 태그 안에는 class 가 link_txt 인 태그 검색, strong 태그 안에는
//            // class 가 link_txt 인 태그는 1개 밖에 없음, first()로 1개 가져옴
//            Element aTag = strong.select(".link_txt").first();
//            // 원하는 태그를 찾았으므로 원하는 테이터 가져오기
