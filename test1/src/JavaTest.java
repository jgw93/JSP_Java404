...
//        System.out.println("\n ------네이버 홈페이지 가져오기--------\n");
//        try {
//            Document naver = Jsoup.connect
//                    ("https://www.naver.com/").get();
...
//        try {
//            res = Jsoup.connect
//                    ("https://www.google.com").method(Connection.Method.GET).execute();
//        } catch (IOException e) {
//            throw new RuntimeException(e);
//        }
//        try {
...
//        }
        ...
        Connection.Response res = null;
        try {
            res = Jsoup.connect("https://www.google.co.kr")
                    .method(Connection.Method.GET).execute();
            doc = res.parse();
        } catch (IOException e) {
            System.out.println("Jsoup로 html 파생 중 오류가 발생했습니다.");
            e.printStackTrace();
        }
...
//        }
        ...
        // select()를 사용하여 지정한 태그를 제외한 나머지 태그를 모두 가져오기
        // 전체 html문서에서 클래스 값이 L3eUgb 인 태그 중 가장 먼저 검색되는
        // 태그를 한개 가져옴
        Element element = doc.select(".L3eUgb").first();
        // 클래스 값이 L3eUgb 인 태그의 자식 태그중 div 태그를 모두 가져옴
        divList1 = element.select("div");
        // 클래스 값이 L3eUgb 인 태그의 자식 태그중 div 태그를 모두 가져오는데
        // 클래스 값이 o3j99 인 태그는 제외함
...
//        System.out.println(divTag);
        ...
        // getElementsByTag() 를 사용하여 지정한 태그를 모두 검색
        Elements divList = doc.getElementsByTag("div");
        System.out.println("divList 수 :" + divList.size());
        // getElementsByClass() 응 사용하여 지정한 Class 값을 가지고 있는 모든 태그를 검색
        divList = doc.getElementsByClass("o3j99");
        System.out.println("o3j99 클래스를 가지고 있는 divList 수 : " + divList.size());

        // Element 객체에서 제공하는 메소드 사용하기
        Element imgTag = doc.select("img").first();
        // attr()로 값 가져오시
        String attrValue = imgTag.attr("src");
        System.out.println("img의 src값 가져오기 : " + attrValue);
        attrValue = imgTag.attr("alt");
        System.out.println("img의 alt값 가져오기 : " + attrValue);
        ...
        System.out.println("className 속성값 : " + classValue);
